...
lastt=$(cat /vagrant/lastrun | sed 's/..........//')
nowt=$(date "+%T")
logt=$(cat /vagrant/access.log | awk '{print $4}' | sed 's/.............//')

# сортировка
cat /vagrant/access.log | sed 's/\[//g' | awk -v last_run="$lastd" -v Now="$nowd" -v lastt_run="$lastt" -v Nowt="$nowt" -v Logt="$logt" \
